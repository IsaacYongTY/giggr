...
describe('addSpaceBetweenChineseWords', () => {
    ...
    it('should retain new line characters and the space around it', () => {
        ...
        expect(
            addSpaceBetweenChineseWords('我该去告诉你\n告诉你真的爱你\n升华彼此间一 段  感情')
        ).toBe('我 该 去 告 诉 你\n告 诉 你 真 的 爱 你\n升 华 彼 此 间 一 段 感 情');
    });
    ...
    it('should return the string unmodified if a word contains A-z characters without space with Chinese characters', () => {
        ...
        expect(addSpaceBetweenChineseWords('maybe我已经睡着 了吗 just maybe')).toBe(
            'maybe我已经睡着 了 吗 just maybe'
        );
    });
    ...
    it('should ignore a complete alphanumeric words', () => {
        ...
        expect(addSpaceBetweenChineseWords('Tommy used to work on the dock')).toBe(
            'Tommy used to work on the dock'
        );
    });

    it('should remove tab characters', () => {
        expect(addSpaceBetweenChineseWords('同样的     天空  下 还是同样     一天')).toBe(
    ...
    it('should remove trailing white spaces of input', () => {
        expect(addSpaceBetweenChineseWords('     告白成功是命中注定     ')).toBe(
            '告 白 成 功 是 命 中 注 定'
        );
    });
});
