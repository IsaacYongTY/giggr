import removeCharacters, { addBackSlashToEscapingCharactersArray } from "../../lib/utils/remove-characters";
...
describe("removeCharacters", () => {
    ...
    describe("addBackSlashToEscapingCharacters", () => {
        it("should add backslash to escaping characters in regex", () => {
            expect(addBackSlashToEscapingCharactersArray([".", "testing"])).toStrictEqual(["\\.", "testing"])
            expect(addBackSlashToEscapingCharactersArray(["*", "|", "another #word"])).toStrictEqual(["\\*", "\\|", "another #word"])
        })
    })
})