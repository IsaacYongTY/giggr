...
import { Musician } from "../lib/types/song";
export default function MusiciansDropdown() {
    ...
    function toggleSelectMusician(musician: Musician) {
        ...
        setSelectedMusicians(prevState => prevState.filter(element => element.id !== musician.id))
    ...
    function handleInput(e: ChangeEvent<HTMLInputElement>) {
        ...
        setFilteredMusicians(musicians.filter(musician => musician.name.toLowerCase().includes(e.target.value) ))
    }
            ...
            <div className={styles.pillButtonRow}>
                {
                    selectedMusicians?.map((composer: any) => (
                        <PillButton key={composer.id} composer={composer} setMusicians={setSelectedMusicians} />
                    ))
                }
            </div>
                ...
                >
                    {
                        filteredMusicians?.map(musician => (
                            ...
                            >
                                ...
                                {
                                    selectedMusicians.findIndex(element => element.id === musician.id) > -1 &&
